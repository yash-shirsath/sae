...
def main(**kwargs):
    ...
    sae_config_kwargs = {
        k: v for k, v in kwargs.items() if hasattr(SaeConfig, k)
    }
    train_config_kwargs = {
    ...
    if 'dtype' in train_config_kwargs:
        ...
        if dtype_str == "float16":
            ...
            print(f"Warning: dtype '{dtype_str}' not recognized or not a torch.dtype. Defaulting to float32. Valid options: float16, float32, bfloat16")
            train_config_kwargs['dtype'] = t.float32
