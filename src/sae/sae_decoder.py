...
def eager_decode(top_indices: t.Tensor, top_acts: t.Tensor, W_dec: t.Tensor):
...
def triton_decode(top_indices: t.Tensor, top_acts: t.Tensor, W_dec: t.Tensor):
    return TritonDecoder.apply(top_indices, top_acts, W_dec)


try:
    from sae.triton_kernels import TritonDecoder
except ImportError:
