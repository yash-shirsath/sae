...
def main(
            ...
            for p_text in current_prompts:  # Renamed 'p' to 'p_text'
                for i, theme in enumerate(styles):
                    if i >= themes_per_prompt:
                        break
                    original_p_text = (
                        p_text  # Store original p_text to avoid modification in loop
                    )
                    if original_p_text.endswith("."):
                        original_p_text = original_p_text[:-1]
                    suffix = (
                        f" in {theme.replace('_', ' ')} style." if theme != "_" else ""
                    )
    ...
    progress_bar = tqdm.tqdm(
        list(concept_prompts_dict.keys()), total=len(concept_prompts_dict)
    )
    ...
    for concept_avail in progress_bar:
        ...
        if not all_prompts_for_concept:
            ...
            concept_latents_dict[concept_avail] = torch.empty(
                (0, steps, d_sae_val), dtype=torch.float16
            )
            continue

        for batch_start_idx in tqdm.tqdm(
            range(0, len(all_prompts_for_concept), batch_size),
            total=len(all_prompts_for_concept) // batch_size if batch_size > 0 else 0,
        ):  # Added check for batch_size > 0
            batch_end_idx = min(
                batch_start_idx + batch_size, len(all_prompts_for_concept)
            )
            ...
            with torch.no_grad():
                for i in range(len(prompts_batch)):
                    ...
                    sae_out_reconstructed = sae_out_sparse.reshape(
                        steps, -1, sae.num_latents
                    ).cpu()

                    mean_sae_features = sae_out_reconstructed.mean(1).to(
                        dtype=torch.float16
                    )
        ...
        if (
            concept_sae_latents_list
        ):  # Corrected class_sae_latents_list to concept_sae_latents_list
            concept_latents_dict[concept_avail] = torch.stack(concept_sae_latents_list)

    os.makedirs(save_dir, exist_ok=True)
    with open(
        os.path.join(save_dir, f"concept_latents_dict_{hookpoint}.pkl"), "wb"
    ) as f:
